...
const InterestRateBooksManager: React.FC = () => {
  ...
  const buildPayload = (status: InterestApprovalState): Omit<InterestRateBook, 'id'> | null => {
    ...
    return {
      ...
      ...(editingRateBook?.checkerName ? { checkerName: editingRateBook.checkerName } : {}),
      ...(editingRateBook?.checkerEmail ? { checkerEmail: editingRateBook.checkerEmail } : {}),
      ...(editingRateBook?.checkerTimestamp ? { checkerTimestamp: editingRateBook.checkerTimestamp } : {}),
      ...(editingRateBook?.publishedBy ? { publishedBy: editingRateBook.publishedBy } : {}),
      ...(editingRateBook?.publishedAt ? { publishedAt: editingRateBook.publishedAt } : {}),
