            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                activities.map((activity) => {
                  ...
                  const serviceTypeLabel = service?.type || activity.serviceType;
                  return (
                    <tr key={activity.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-gray-700">{serviceTypeLabel || 'Unknown'}</td>
            ...
            <form onSubmit={handleSaveActivity}>
              ...
              <div className="mb-4">
                ...
                <select name="serviceId" id="serviceId" value={formData.serviceId} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500" required>
                  ...
                  {services.map(s => <option key={s.id} value={s.id}>{s.type ? `${s.type} — ${s.name}` : s.name}</option>)}
                </select>
              </div>
              <div className="mb-4">
